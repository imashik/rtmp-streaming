...
public class Camera2ApiManager extends CameraDevice.StateCallback {
        ...
        try {
            ...
            if (cameraCharacteristics.get(CameraCharacteristics.LENS_FACING) == facing) {
                ...
                for (int i = 0; i < cameraIdList.length; i++) {
                    if (cameraManager.getCameraCharacteristics(cameraIdList[i]).get(CameraCharacteristics.LENS_FACING) == facing) {
                        selectedCameraId = i;
                        break;
                    }
                }
                openCameraId(selectedCameraId);
            }
        } catch (CameraAccessException e) {
            Log.e(TAG, "Error", e);
        }
    ...
    public void switchCamera() {
        if (cameraDevice != null) {
            int cameraId = Integer.parseInt(cameraDevice.getId()) == 0 ? 1 : 0;
            closeCamera(false);
            prepared = true;
            openCameraId(cameraId);
        }
